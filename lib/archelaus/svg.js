...
svg.addEventListener('mousemove', function(ev) {
  ...
  if (es) {
    menu.querySelector('.xy')
      .textContent = es[0].replace(/,/, '/') + ' ' + x + 'm/' + y + 'm';
    menu.querySelector('.latlon')
      .textContent = es[1] + ' ' + es[2];
    menu.querySelector('.elevation')
      .textContent = (es[3] === 's') ? 'sea' : ('ele ' + es[3] + 'm');
  }
  menu.querySelector('.text').textContent = t;
});
