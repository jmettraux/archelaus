...
//  updateMenuText(ta);
  ...
  if (pre) {
    ...
    var x0 = pre.getAttribute('x');
    var y0 = pre.getAttribute('y');
    var x1 = ta.getAttribute('x');
    var y1 = ta.getAttribute('y');
    ...
    var i = 'p' + Date.now();
    createSvgElement(
      'path', { id: i, class: 'ruler', d: d });
    var te = createSvgElement(
      'text', {});
    createSvgElement(
      te, 'textPath', { href: '#' + i, class: 'ruler' }, '' + m + ' meters');
  }
  else if (pre !== ta) {
